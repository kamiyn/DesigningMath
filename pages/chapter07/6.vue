...
<script lang="ts">
/* http://localhost:3000/chapter07/6 */
...
export default defineComponent({
  ...
  methods: {
    ...
    loopFunc(
      ...
      const fureKakudo = Math.sin(second * Math.PI * 2) * Math.PI * 0.2; // 1秒周期で10% = 36度ゆらぐ
      ...
      maruArrArr.forEach((maruY) =>
        maruY.forEach((maru) => {
          ...
          const x1 = hankei * Math.cos(kakudo + fureKakudo);
          const y1 = hankei * Math.sin(kakudo + fureKakudo);
          ctx.beginPath();
